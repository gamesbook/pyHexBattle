...
def help():
    print("Please supply a scenario filename, e.g. PegasusBridge.xml, with a -f in front:")
    print('  python memoir.py -f <scenario_file> -d <display_file>')
    print('     OR ')
    print("Supply a scenario directory, e.g. /home/derek/Desktop, with a -s in front:")
    print('  python memoir.py -s <scenario_directory>')    
...
def main(argv):
    ...
    dom = etree.parse("Memoir/scenarios/{}".format(inputfile))
    if the_style == 'bw':
        xslt = etree.parse("Memoir/styles/battle2bwSVG.xsl")
    elif the_style == 'terrain':
        xslt = etree.parse("Memoir/styles/battle2colorTerrain.xsl")
    else:
        xslt = etree.parse("Memoir/styles/battle2colorSVG.xsl")
    transform = etree.XSLT(xslt)
    result = transform(dom)

    cwd = os.getcwd()
    _path = os.path.join(cwd, 'Memoir')
